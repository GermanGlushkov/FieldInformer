...
namespace FI.DataAccess
{
	...
	/// </summary>
    public class CustomSqlReports : MarshalByRefObject, FI.Common.DataAccess.ICustomSqlReportsDA
	{
		...
		{
			FI.Common.Data.FIDataTable data=new FI.Common.Data.FIDataTable();
            DataBase.Instance.ExecuteCommand(Sql, GetOltpConnString(Database), CommandType.Text, null, data);
			return data;
		}


        public object ExecuteScalarOLTP(string Sql, string Database)
        {
            return DataBase.Instance.ExecuteScalar(Sql, GetOltpConnString(Database), null);
        }

        private string GetOltpConnString(string database)
        {
            string connString = FI.Common.AppConfig.DA_OltpConnectionString;
            if (database == null || database == "")
            {
                string newConnStr = "";
                string[] items = connString.Split(new char[] { ';' });
                foreach (string item in items)
                    if (!item.Trim().StartsWith("Database", StringComparison.OrdinalIgnoreCase))
                        newConnStr += (item + ";");

                return newConnStr;
            }
            else
                return connString.Replace("@DATABASE", database);
        }
