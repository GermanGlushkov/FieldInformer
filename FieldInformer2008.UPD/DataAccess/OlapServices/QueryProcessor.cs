...
namespace FI.DataAccess.OlapServices
{
	...
	public class QueryProcessor:MarshalByRefObject, IDisposable
	{
		...
		internal QueryProcessor(string Server, string Database)
		{
			_server=Server;
			_database=Database;
		}
		...
		{
			get { return (_allocatedOn==DateTime.MinValue ? TimeSpan.MaxValue : DateTime.Now.Subtract(_allocatedOn));}
		}
		...
		internal bool TryAllocate(string taskId)
		{
			lock(this)
			{
                if (this.IsAllocated)
                {
                    if (AllocatedSpan.Seconds > Common.AppConfig.DA_MdxCommandTimeout)
                        Release(_taskId, true);
                    else
                        return false;
                }

				if(taskId==null)
                    throw new ArgumentNullException("taskId cannot be null");
			
				if(ProcessorPool.Instance.IsTaskCanceled(taskId))
					throw new Exception("Task is canceled: " + taskId);
		...
		private object ExecXmlCstCommand(System.Delegate commandDelegate, object[] args)
		{
            ...
            catch (Exception exc)
            {
                ...
                // write event log
                FI.Common.LogWriter.Instance.WriteEventLogEntry((exc.InnerException != null ? exc.InnerException : exc));

                // throw as generic exception, because it can be deserialized in dotnet1.0
                throw new Exception(exc.InnerException != null ? exc.InnerException.Message : exc.Message);
            }
            finally
            {
                lock(this)
        ...
        private delegate DateTime GetCubeLastProcessedDelegate(string Server, string Database, string Cube);
        public DateTime GetCubeLastProcessed(string Cube)
        {
            GetCubeLastProcessedDelegate commandDelegate = new GetCubeLastProcessedDelegate(_adomd.GetCubeLastProcessed);
            return (DateTime)ExecXmlCstCommand(commandDelegate, new object[] { Server, Database, Cube });
        }
