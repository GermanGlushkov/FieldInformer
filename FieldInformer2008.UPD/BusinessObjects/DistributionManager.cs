...
//					System.Web.Mail.SmtpMail.Send(msg);					
		...
		{			
			...
			try
			{
                ...
                // send report
                User user = new User(userId, false); //not a proxy cause oltp db name is required
...
//						dacObj.EnqueueDistribution(distributionId, "Requeue after error");
        ...
        public void RefreshCachedReports(string CompanyNameShort)
        {
            ...
            try
            {
                ...
                if (tbl != null)
                    ...
                    {
                        User usr = new User((decimal)row["user_id"], false); //not a proxy cause oltp db name is required
		...
		public void EnqueueScheduledDistributions(System.DateTime Date , string CompanyNameShort)
        {
			...
			foreach(System.Data.DataRow row in table.Rows)
			{
				...
				{
                    User user = new User((decimal)row["Id"], false); //not a proxy cause oltp db name is required
