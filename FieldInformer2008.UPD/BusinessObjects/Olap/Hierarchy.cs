...
namespace FI.BusinessObjects.Olap
{
	...
	public class Hierarchy:Object
	{
		...
		internal void LoadFromXmlSchema(System.Xml.XmlElement xmlEl)
		{
			...
			_displayFolder=xmlEl.GetAttribute("F");
...
*/
		...
		public string DisplayName
		{
			get 
			{ 
				string s=(this.Dimension.Hierarchies.Count==1 && this.Name==this.Dimension.Name ? "" : this.Dimension.Name + ".");
				return s + this.Name; 
			}
		}
		...
		public string DisplayPath
		{
			get 
			{
				return (this.DisplayFolder!="" ? this.DisplayFolder + "." : "") + (this.Dimension.Name) + "." + this.Name;
			}
		}
		...
		public void Close()
		{
			this._isOpen=false;
			// update open nodes
			Schema.CloseNode(this.UniqueName);
		}
...
//				this.CalculatedMembers.RemoveAt(0);
	...
	{
		private class DisplayPathComparer:IComparer
		{
			// singleton pattern
			private DisplayPathComparer(){}
			public static readonly DisplayPathComparer Inst=new DisplayPathComparer();
			// singleton pattern

			public int Compare(object x, object y)
			{
				return string.Compare( ((Hierarchy)x).DisplayPath, ((Hierarchy)y).DisplayPath, true);
			}
		}
		...
		public Hierarchy[] ToSortedByDisplayPathArray()
		{
			Hierarchy[] ret=this.ToArray();
			Array.Sort(ret, DisplayPathComparer.Inst);
			return ret;
		}
