...
namespace FI.BusinessObjects.Olap
{
	...
	public class Schema
	{
		...
		public bool IsNodeOpen(string id)
		{
			return _openNodes.Contains(id);
		}

		public void OpenNode(string id)
		{
			if(_openNodes.Contains(id)==false)
				_openNodes.Add(id);
		}

		public void CloseNode(string id)
		{
			_openNodes.Remove(id);
		}

		public void OpenNodesFromXml(string openNodesXml)
		{
			// discard open nodes
			_openNodes.Clear();
		...
		public string OpenNodesToXml()
		{
			XmlDocument doc=new XmlDocument();
			XmlElement rootEl=doc.CreateElement("NODES");
			doc.AppendChild(rootEl);

			foreach(string openNodeUn in _openNodes)
			{
