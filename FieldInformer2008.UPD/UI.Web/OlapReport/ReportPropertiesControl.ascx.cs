namespace FI.UI.Web.OlapReport
{
	...
	public partial class ReportPropertiesControl : System.Web.UI.UserControl
	{
		...
		protected override void Render(System.Web.UI.HtmlTextWriter writer)
		{
            ...
            filteredSetCell.Visible = isOpen;
			...
			if(isOpen)
			{
                ...
                // sel filtered set levels
                foreach (Dimension dim in _report.Schema.Dimensions)
                {
                    if (dim.UniqueName == "[Measures]")
                        continue;
                    foreach (Hierarchy hier in dim.Hierarchies)
                        if(hier.Axis.Ordinal!=2) // only row or column hierarchies
                            foreach (Level lev in hier.Levels)
                                if(!lev.IsAllLevel) //skip all level
                                    this.selFilteredSetLevel.Items.Add(lev.UniqueName);
                }

                // sel filtered set operator
                string[] items = Enum.GetNames(typeof(FI.BusinessObjects.Olap.CalculatedMemberTemplates.FilteredByMeasureSet.Operators));
                foreach(string item in items)
                    selFilteredSetOperator.Items.Add(item);
		...
		private void AddSelCalcMeasureWithChildren(SchemaMember smem)
		{
			...
			{
                selFilteredSetMeasure.Items.Add(smem.Name);
        ...
        public void OnUpdateClick()
        {
            ...
            {
                UpdateReportHeader();
                if (Request.Form[this.UniqueID + ":selRowEmpty"] == null)
                    return;
