...
namespace FI.UI.Web.OlapReport
{
	...
	public class Controller
	{
		...
		*/
        ...
        public void AddFilteredByMeasureSet(string levUN, string meaName, string op, string value)
        {
            Level lev = _report.Schema.Levels[levUN];
            if (lev == null)
                throw new Exception("Unable to find level: " + levUN);
            Member mea = _report.Schema.Hierarchies["[Measures]"].SchemaMembers.Find("[Measures].[" + meaName.Replace("]", "]]") + "]");
            if (mea == null)
                throw new Exception("Unable to find measure: " + meaName);
            FilteredByMeasureSet.Operators opEnum = (FilteredByMeasureSet.Operators)Enum.Parse(typeof(FilteredByMeasureSet.Operators), op);

            FilteredByMeasureSet set = new FilteredByMeasureSet(null, lev, mea, opEnum, value);
            lev.Hierarchy.AddMember(set, true);
        }
