...
function sort(field)
{
	...
	if (xmlDoc.selectSingleNode(""/root/Schema/ElementType[@name='row']/AttributeType[@name='""+ field.replace(/@/, """")  +""']/datatype[type='i8' or @type='number' or @type='float'  or @type='int'  or @type='r4'  or @type='i2'  or @dt:type='ui1' ]/@type"") )
...
<xsl:for-each select=""/root/Schema/ElementType[@name='row']/AttributeType"">
    			<TH onclick=""sort('@{@name}')"" NOWRAP='True'>
    			<xsl:value-of select=""@name"" />
    			<xsl:if test=""@name=$cur_sort_field"">
...
position()  &lt;= number($recordsPerPage * number($pageNumber) + $recordsPerPage))"">
		...
		<xsl:variable name=""col_name"" select=""name()""/>
				...
				<xsl:choose>
   					<xsl:when test=""/root/Schema/ElementType[@name='row']/AttributeType[@name=$col_name]/datatype[@type='numeric'] "">
