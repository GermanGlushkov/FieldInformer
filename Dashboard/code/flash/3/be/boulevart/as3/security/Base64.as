package be.boulevart.as3.security { 
	...
	*/
	...
	public class Base64 {
		...
		* Decodes a base64 string.
		*/
		public static function decodeToHex(src:String):String {
			var i:Number = 0;
			var output:String = new String("");
			...
			while (i < src.length) {
				...
				chr2 = ((enc2 & 15) << 4) | (enc3 >> 2);
				chr3 = ((enc3 & 3) << 6) | enc4;
				output += hexes[chr1 >> 4] + hexes[chr1 & 0xf];
				if (enc3 != 64) output = output+hexes[chr2 >> 4] + hexes[chr2 & 0xf];
				if (enc4 != 64) output = output+hexes[chr3 >> 4] + hexes[chr3 & 0xf];
			}
			return output;
		}
	}
