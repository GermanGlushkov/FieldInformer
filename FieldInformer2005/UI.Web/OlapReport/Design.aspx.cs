	...
	public class Design : PageBase
	{
		...
		private void ExecuteCommands()
		{
			...
			for(int i=0;i<Request.Form.Count;i++)
			{
				...
				else if(key.StartsWith(_selControl.UniqueID + ":sel_dopen:"))
				{
					int index=key.IndexOf(":");
					index=key.IndexOf(":" , index+1)+1;
					string dimUn=key.Substring(index , key.Length-index);
					
					try
					{
						_contr.OpenDimension(dimUn);
					...
					_pageScrollId=key.Replace("sel_dopen" , "sel_dclose"); //it will hage it's id
					break;
				}
				else if(key.StartsWith(_selControl.UniqueID + ":sel_dclose:"))
				{
					int index=key.IndexOf(":");
					index=key.IndexOf(":" , index+1)+1;
					string dimUn=key.Substring(index , key.Length-index);
					...
					_pageScrollId=key.Replace("sel_dclose" , "sel_dopen"); //it will hage it's id
					break;
				}
