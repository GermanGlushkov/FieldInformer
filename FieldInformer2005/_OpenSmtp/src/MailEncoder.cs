...
using System.Text;
	...
	public class MailEncoder
	{
		...
		{
			return ConvertToQP(s, charset, true);
		}
		...
		public static string ConvertToQP(string s, string charset, bool skipLineLengthRule)
		{
			//NB!!! 
			// BY GER : LineLengthRule cannot be normally parsed by OUTLOOK?!
			...
			for(int sidx=0; sidx < s.Length; sidx++)
			{
				...
				if (!skipLineLengthRule && currLine.Length >= 76)
				{
		...
		/// <returns></returns>
		public static string ConvertHeaderToQP(string s, string charset, bool forceconversion)
		{
			if (!forceconversion) 
			{
			...
			return "=?" + charset + "?Q?" + ConvertToQP(s, charset, true) + "?=";
		}
		...
		/// <param name="charset"></param>
		/// <returns></returns>
		public static string ConvertHeaderToQP(string s, string charset) 
		{
		...
		/*
		 * commented out by GEr, cause ConvertToQP was modified
		public static string ConvertFromQP(string s)
		{
			if (s == null) return null;
