namespace FI.UI.Web.StorecheckReport
{
	...
	/// </summary>
	public class ReportLoadControl : System.Web.UI.UserControl
	{
		...
		protected System.Web.UI.HtmlControls.HtmlTable paramsTable;
		...
		{
			_report=(FI.BusinessObjects.StorecheckReport)_user.ReportSystem.GetReport(Id , typeof(FI.BusinessObjects.StorecheckReport) , true);
		...
		private void RedirectToReport()
		{
			...
			if(_action=="Open" && FI.Common.AppConfig.ReportRunOnClick)
				Response.Redirect(Request.ApplicationPath + "/StorecheckReport/Table.aspx?content=NotDelivered" , true);
			else
				Response.Redirect(Request.ApplicationPath + "/StorecheckReport/Design.aspx" , true);
		}
