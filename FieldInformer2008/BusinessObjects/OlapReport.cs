...
namespace FI.BusinessObjects
{
	...
	public class OlapReport:Report
	{
		...
		public enum GraphTypeEnum
		{
			BarVertical=0,
			BarHorizontal=1,
			StackedBarVertical=2,
			StackedBarHorizontal=5,
            LineHorizontal = 3,
            LineWithBars = 7,
			Pie=4,
            PieBrokenApart = 6,
		...
		protected internal string _graphTheme;
		protected internal GraphOptionsEnum _graphOptions=GraphOptionsEnum.None;
		protected internal short _graphWidth;
		protected internal short _graphHeight;
        protected internal int _graphNum1;
		...
		*/
		...
		override protected internal Report _Clone(string Name , string Description)
		{
			...
			string OpenNodesXml=this.Schema.OpenNodesToXml();

			decimal newId=dacObj.InsertReport(_owner.ID , 0 , 0 , Name , Description , false , 
				(byte)this.GraphType , this.GraphTheme, (int)this.GraphOptions , this.GraphWidth, this.GraphHeight, this.GraphNum1,
				ReportXml , OpenNodesXml);
...
//			dacObj.SaveReport(_owner.ID , this._id  , SaveToXml(), this.SaveOpenNodesToXml() );
		...
		// ------------- async execute ------------- 
		private delegate string ExecuteDelegate(string Server, string Database, string Cube, string Mdx, string TaskGuid, string TaskDescription, string ReportType, decimal ReportId);
		...
		protected internal override void _BeginExecute()
		{
			...
			string taskDescription=string.Format("User: {0} ({1}), OlapReport: {2} ({3})",
            ...
            _asyncResult = _asyncDelegate.BeginInvoke(Schema.Server, Schema.Database, Schema.Cube, _mdx, _olapTaskGuid.ToString(), taskDescription, "Olap", this.ID, null, null);
		}
		...
		*/
        ...
        private int GraphNum1
        {
            get { return _graphNum1; }
            set { _graphNum1 = value; }
        }

        // derived from GraphNum1
        public byte GraphPieColumns
		{
            get { return Convert.ToByte(this.GraphNum1); }
            set
            {
                if (this.GraphType != GraphTypeEnum.Pie && this.GraphType != GraphTypeEnum.PieBrokenApart)
                    return;

                this.GraphNum1 = value; 
            }
		}

        // derived from GraphNum1
        public byte GraphMixedLinePosition
        {
            get { return Convert.ToByte(this.GraphNum1); }
            set 
            {
                if (this.GraphType != GraphTypeEnum.LineWithBars)
                    return;

                this.GraphNum1 = (value==0 ? (byte)1 : value); 
            }
        }  
