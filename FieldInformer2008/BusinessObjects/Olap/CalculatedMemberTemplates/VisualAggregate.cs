		...
		public override string MDXDefinitions
		{
			get
			{
				...
				else
				{
                    ...
                    // current memmber for other hierarchies in same dimension
                    foreach (Hierarchy hier in _hierarchy.Dimension.Hierarchies)
                        if (hier.UniqueName != _hierarchy.UniqueName)
                            sb.Append(", " + hier.UniqueName + ".CurrentMember"); 
